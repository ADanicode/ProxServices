...
// Estado de la UI para la pantalla de inicio del cliente
...
class ClientHomeViewModel(
    ...
    private fun fetchUserName() {
        viewModelScope.launch {
            ...
            result.fold(
                onSuccess = { name ->
                    _uiState.value = ClientHomeState(isLoading = false, userName = name)
                },
                onFailure = {
                    _uiState.value = ClientHomeState(isLoading = false, userName = "Usuario")
                }
            )
        }
    }
    ...
    class Factory(
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            if (modelClass.isAssignableFrom(ClientHomeViewModel::class.java)) {
                ...
                return ClientHomeViewModel(userId, authRepository) as T
            }
